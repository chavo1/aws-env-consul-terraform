...
resource "aws_instance" "client" {
  ...
  key_name                    = "${data.terraform_remote_state.client.outputs.key_name_dc2[0]}"
  iam_instance_profile        = "${data.terraform_remote_state.client.outputs.aws_iam_instance_profile_dc2[0]}"
  associate_public_ip_address = true
  vpc_security_group_ids      = ["${data.terraform_remote_state.client.outputs.aws_security_group_ohio[0]}"]
